...
<section>
    <div class="grid grid-cols-12 gap-4">
        ...
        <div class="bg-white border rounded-lg p-4 md:col-span-4 col-span-12 open-rfid-modal cursor-pointer">
            <div class="flex h-16 w-16 text-2xl items-center justify-center rounded-full bg-meta-2 dark:bg-meta-4">
    ...
    <!-- Modal -->
    <div id="rfidModal" class="fixed inset-0 bg-black/70 z-99999 hidden items-center justify-center">
...
<script>
    ...
    // Open Modal
    $('.open-rfid-modal').on('click', function(e) {
        e.preventDefault();
        $('#confirmDeleteBtn').attr('href', '');
        $('#rfidModal').removeClass('hidden').addClass('flex');
    });

    // Close Modal
    $('#cancelModal').on('click', function() {
        $('#rfidModal').addClass('hidden').removeClass('flex');
    });

    $('#rfidModal').on('click', function(e) {
        if (e.target === this) {
            $('#rfidModal').addClass('hidden').removeClass('flex');
        }
    });
