        ...
        <form action="<?= base_url('production/inventory/add/process') ?>" method="post" enctype="multipart/form-data">
            <div>
                ...
                <div class="mb-4.5 flex flex-col gap-4" id="wrapper-category">
                    ...
                    <?php if (isset($input['category'])) {
                        foreach ($input['category'] as $val) {
                    ?>
                            <div class="flex flex-row gap-4" id="item-category">
                                <input type="text" name="category[]" value="<?= $val ?>" placeholder="Kategori" class="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter" />
